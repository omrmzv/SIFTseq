...
def editfastq(r1_in, r2_in, model, remove_or_tag, count_or_filter, ignore_motif, output_name, output_dir, report_filename):
	...
	if count_or_filter == 'f':
		...
		pd.DataFrame(list_of_rows).to_csv(output_dir + '/'+report_filename)

	else:
		# If just counting, remove the predictions column from the dataframe
		df = pd.DataFrame(list_of_rows)
		df[['read_id','C_count','T_count']].to_csv(output_dir + '/'+report_filename)
...
def edit_tblat(tblat_file, model, count_or_filter, remove_or_tag, ignore_motif, output_name, output_dir, report_filename, metagenomic_reference, r1, r2):
	"""
	...
	# Assumes there are R1 and R2 fastq files with the same name as the tblat file with tags _R1.fq.gz and _R2.fq.gz
	#r1 = '.'.join(tblat_file.split('.')[0:-1])+'_R1.fastq.gz'
	#r2 = '.'.join(tblat_file.split('.')[0:-1])+'_R2.fastq.gz'
	...
	with open(tblat_file) as f, dnaio.open(r1) as fqr1, dnaio.open(r2) as fqr2:
		...
		#result = Parallel(n_jobs=2) (delayed(mismatch)(entry, record1, record2, fasta) for entry, record1, record2 in zip(f, fqr1, fqr2) )
	#print(rows)
	...
	if count_or_filter == 'f':
		...
		species = df[["taxid", "classification"]]
		species = species.groupby(['taxid'])['classification'].mean().reset_index()
		species = species.rename(columns = {'classification' : 'species_contam_proba'})

		df = pd.merge(df, species, on = 'taxid')
		...
		df['pass'] = (df['species_contam_proba']>0.5).astype(int)
		...
		if remove_or_tag == 't':
			...
			df.to_csv(output_dir+'/tagged.'+output_name)
		else: # if remove, remove all entries where the species are contaminated
			print(f"Removed {fp} ({perc}%) contaminated reads")
			fail = df[df['pass']==1]
			pas = df[df['pass']==0]
			fail.to_csv(output_dir+'/removed.'+output_name, sep = '\t', index=False)
			pas.to_csv(output_dir+'/'+output_name, sep = '\t', index=False)
	...
	else: # Just want the counts, can simply save part of the df as new file
		df[['read_id','C_count','T_count']].to_csv(output_dir+'/'+report_filename, sep = '\t', index=False)
...
if __name__ == '__main__':
	...
	"python3 filter3.py -fn BK_short.tblat -ft tblat -mo model1.joblib -cf f -rt t -o test1.tblat -r report_test1.csv -mr NCBIGenomes06.fna"

	"python3 filter3.py -fn BK_short_R1.fastq.gz BK_short_R2.fastq.gz -ft fastq -mo model1.joblib -cf c -o test1.fastq.gz -r report_test1.csv"
	"""))
	...
	parser.add_argument('-o',"--output_name", type=str, help = 'Path for edited files',nargs=1)
	...
	if file_type == 'fastq':
		...
		for r1i, r2i in zip(r1in, r2in):
			start = time.time()
			editfastq(r1i, r2i, model, remove_or_tag, count_or_filter, ignore_motif, output_name, output_dir, report_filename)
			end = time.time()
		...
		for bam_file in rs:
			start = time.time()
			edit_bam(bam_file, model, remove_or_tag, count_or_filter, output_name, output_dir, ignore_motif, report_filename)
			end = time.time()
		...
		tblat = rs[2]
		start = time.time()
		edit_tblat(tblat, model, count_or_filter, remove_or_tag, ignore_motif, output_name, output_dir, report_filename, metagenomic_reference, r1, r2)
		end = time.time()
