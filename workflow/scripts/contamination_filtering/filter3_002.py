...
def get_cytosines(r1_start, r1_end, r2_start, r2_end, R1, qual1, R2, qual2, reference_sequence, strand):
...
def get_reference_seq(sseqid, ref_r1_start, ref_r1_end, ref_r2_start, ref_r2_end, frag_length, fasta, strand, genome_length):
...
def mismatch(entry, record1, record2, fasta):
	...
	R1 = record1.sequence
	Q1 = record1.qualities
	R2 = record2.sequence
	Q2 = record2.qualities
	reference_sequence = get_reference_seq(sseqid, int(ref_r1_start), int(ref_r1_end), int(ref_r2_start), int(ref_r2_end), int(float(frag_length)), fasta, strand, int(float(genome_length)))

	mismatch_string, molecule, ref_seq = get_cytosines( int(r1_start), int(r1_end), int(r2_start), int(r2_end), R1, Q1, R2, Q2, reference_sequence, strand )


	new_entry = entry.strip('\n').split('\t')+[mismatch_string, molecule, ref_seq]
	return(new_entry)


def mismatch2(row, fasta):
	#fasta = Fasta(metagenomic_reference)
	read_id = row['qseqid']
	strand = row['strand']
	sseqid = row['sseqid']

	r1_start = row['qstart_R1']
	r1_end = row['qend_R1']
	ref_r1_start = row['sstart_R1']
	ref_r1_end = row['send_R1']

	r2_start = row['qstart_R2']
	r2_end = row['qend_R2']
	ref_r2_start = row['sstart_R2']
	ref_r2_end = row['send_R2']
	frag_length = row['effective_length']
	genome_length = row['genome_len']
	...
	mismatch_string, molecule, ref_seq = get_cytosines( int(r1_start), int(r1_end), int(r2_start), int(r2_end), R1seq, R1qual, R2seq, R2qual, reference_sequence, strand )


	#new_entry = entry.strip('\n').split('\t')+[mismatch_string, molecule, ref_seq]
	row['mismatch_string'] = mismatch_string
	row['molecule'] = molecule
	row['reference_sequence'] = ref_seq

	return(row)

def mismatch3(entry, record1, record2, fasta):
	read_id = entry.split('\t')[1]
	strand = entry.split('\t')[2]
	sseqid = entry.split('\t')[3]
...
def process_chuncks(chunck, metagenomic_reference):
	...
	t = time.time()-s
	s= time.time()
	print("loading fasta took " + str(t))
	a = {}
	i=0
	print("chunky monkey")
	...
	for line in chunck:
		...
		a[i] = mismatch3(entry, record1, record2, fasta)
	a = pd.DataFrame.from_dict(a, orient='index')
	print("processing " + str(len(chunck))+ " lines took " + str(time.time()-s))
	return(a)

def process_chunck2(chunck, metagenomic_reference):
	s=time.time()
	fasta=Fasta(metagenomic_reference)
	chunck = chunck.apply(mismatch2, axis=1, fasta = fasta)
	print('processed chunk that had ' + str(len(chunck.index)) + ' rows in ' + str(time.time()-s) + ' s')
	return(chunck)

def edit_tblat(tblat_file, r1, r2, model, count_or_filter, remove_or_tag, ignore_motif, output_name, output_dir, report_filename, metagenomic_reference, LUT):
	"""
	...
	#
	#
	# tblat_file_df = pd.read_csv(tblat_file, sep ='\t')
	# R1 = pd.read_csv('R1.test', sep = '\t', header=None, names = ['qseqid', 'seqR1', 'plus', 'qual'] )
	# R2 = pd.read_csv('R2.test', sep = '\t', header=None, names = ['qseqid', 'seqR2', 'plus', 'qual'] )
	#
	# tblat_file_df["seqR1"] = R1["seqR1"]
	# tblat_file_df["qualR1"] = R1["qual"]
	# tblat_file_df["seqR2"] = R2["seqR2"]
	# tblat_file_df["qualR2"] = R2["qual"]
	# chuncks = np.array_split(tblat_file_df, 3000)
	# print("processed input in ")
	# print(time.time()-start)
	# start = time.time()
	# df_list = Parallel(n_jobs=10)(delayed(process_chuncks)(chunck, metagenomic_reference) for chunck in chuncks[0:30])
	# print("done in")
	# print(time.time()-start)
	...
	with open(tblat_file) as f, dnaio.open(r1) as fqr1, dnaio.open(r2) as fqr2:
		...
		for entry, record1, record2 in zip(f,fqr1, fqr2):
			row.append([entry, record1,record2])
	print(time.time()-start)
	print("Creating chuncks")
	chuncks = chunkIt(row, 300)

	print("Processing chuncks")
	start = time.time()
	df_list = Parallel(n_jobs=10)(delayed(process_chuncks)(chunck, metagenomic_reference) for chunck in chuncks)

	print("Time for chuncks")
	print(time.time()-start)


	tt = Parallel(n_jobs=10)(delayed(mismatch3)(entry, record1, record2, refe) for entry, record1, record2, refe in zip(rows, r1, r2, a))
	print("Time for lines")
	print(time.time()-start)
	df = pd.DataFrame.from_dict(dict, orient='index')

	tblat_file_df = pd.read_csv(tblat_file, sep ='\t')
	R1 = pd.read_csv(r1, sep = '\t', header=None, names = ['qseqid', 'seqR1', 'plus', 'qual'] )
	R2 = pd.read_csv(r2, sep = '\t', header=None, names = ['qseqid', 'seqR2', 'plus', 'qual'] )

	#### TEST AN RBIND APPLY WITH MISMATCH2 (write mismatch2 so it actually works)

	tblat_file_df["seqR1"] = R1["seqR1"]
	tblat_file_df["qualR1"] = R1["qual"]
	tblat_file_df["seqR2"] = R2["seqR2"]
	tblat_file_df["qualR2"] = R2["qual"]
	#tblat_file_df['FA'] = fasta

	#print("Trying dask")
	#start = time.time()
	#dask_meta_df = pd.DataFrame().reindex_like(tblat_file_df)
	#dask_meta_df['mismatch_string'] = "a"
	#dask_meta_df['molecule'] = "a"
	#dask_meta_df['reference_sequence'] = "a"
	#ddata = dd.from_pandas(tblat_file_df, npartitions=10)
	#df2 = ddata.apply(mismatch2, axis=1, meta=dask_meta_df).compute(scheduler="multiprocessing")

	print("trting MP")
	start = time.time()
	df2 = parallelize_on_rows(tblat_file_df, mismatch2)
	print(time.time()-start)
	#print("trying pandarallel")
	#start = time.time()
	#df = tblat_file_df.parallel_apply(mismatch2, axis=1, fasta=fasta)
	#print(time.time()-start)
	print(df2.head())

	print("Regular apply")
	start = time.time()
	df = tblat_file_df.apply(mismatch2, axis=1)
	print(time.time()-start)

	# Convert list to df:
	#df = pd.DataFrame(rows, columns = ['taxid', 'qseqid', 'strand', 'sseqid', \
...
if __name__ == '__main__':
	...
	if file_type == 'fastq':
		...
		tblat = rs[2]
		start = time.time()
		edit_tblat(tblat, r1, r2, model, count_or_filter, remove_or_tag, ignore_motif, output_name, output_dir, report_filename, metagenomic_reference, LUT)
		end = time.time()
